...
function M.set(cache_type, key, data)
  ...
  if (M.metadata[cache_type].hits + M.metadata[cache_type].misses) % 10 == 0 then
    vim.defer_fn(function() M.save_cache() end, 1000)
  end
...
function M.prune(cache_type)
  ...
  table.sort(entries, function(a, b) return a.accessed < b.accessed end)
...
function M.show_stats()
  ...
  -- Header
  table.insert(lines, string.format("%-10s %-8s %-8s %-8s %-10s", 
                                   "Cache", "Size", "Hits", "Misses", "Hit Rate"))
  table.insert(lines, string.rep("-", 50))
  
  -- Rows
  for cache_type, meta in pairs(stats.metadata) do
    table.insert(lines, string.format("%-10s %-8d %-8d %-8d %-10.1f%%", 
                                    cache_type, meta.size, meta.hits, meta.misses, meta.hit_rate))
  end
...
function M.setup(api)
  ...
  api.get_issue = M.wrap('issues', 
    function(issue_key) return issue_key end,
    api.get_issue_orig
  )
  
  -- Cache project data
  api.get_project_orig = api.get_project
  api.get_project = M.wrap('projects',
    function(project_key) return project_key end,
    api.get_project_orig
  )
  
  -- Cache user search
  api.find_users_orig = api.find_users
  api.find_users = M.wrap('users',
    function(query) return "query:" .. query end,
    api.find_users_orig
  )
  
  -- Cache board data
  api.get_boards_orig = api.get_boards
  api.get_boards = M.wrap('boards',
    function() return "all_boards" end,
    api.get_boards_orig
  )
  
  -- Cache project boards
  api.get_project_boards_orig = api.get_project_boards
  api.get_project_boards = M.wrap('boards',
    function(project_key) return "project_boards:" .. project_key end,
    api.get_project_boards_orig
  )
  
  -- Cache sprints for a board
  api.get_sprints_orig = api.get_sprints
  api.get_sprints = M.wrap('sprints',
    function(board_id) return "board_sprints:" .. board_id end,
    api.get_sprints_orig
  )
  
  -- Cache issue search results
  api.search_issues_orig = api.search_issues
  api.search_issues = M.wrap('search',
    function(jql, max_results) return "jql:" .. jql .. "|" .. (max_results or 50) end,
    api.search_issues_orig
  )
