...
function M.format_issue_hover_info(issue)
  ...
  table.insert(
    lines,
    '_Actions:_ [View](' .. api.get_browse_url(issue.key) .. ') | Run `:JiraIssueView ' .. issue.key .. '`'
  )
      ...
      api.get_issue(issue.key, function(err, issue_data)
        if not err and issue_data then
          ...
          if
            ...
            and type(issue_data.fields.assignee) == 'table'
            and issue_data.fields.assignee.displayName
          then
